...
public class ResultsJSONParser {
    public static void resultsParse(String str, TextView mTitleTextView, TextView mCBTextView, TextView mDateCBTextView, TableLayout mTableLayout, Context context, boolean isTablet){
        try {
            ...
            if(isTablet)mCBTextView.setTextSize(30);
            ...
            for (int a=0; a<jsonArray.length(); a++){
                ...
                if (subObject.isNull("results")){
                    subElementsParse(subObject, mTableLayout, context, isTablet);
                }
                else{
                    ...
                    for (int s=0; s<jsonSubArray.length(); s++){
                        ...
                        subElementsParse(itemObject, mTableLayout, context, isTablet);
                    }
    ...
    static void subElementsParse(JSONObject jObject, TableLayout mTableLayout, Context context, boolean isTablet) {
        ...
        if (result.equals("Repairs Needed")){
            ...
            if(isTablet)repairRow.setTextSize(25);
        }
        else if (result.equals("Missing")) {
            resultView.setTextColor(Color.RED);
            if (jObject.optString("note")!=null){
                TextView repairRow = new TextView(context);
                repairRow.setPadding(10,0,10,0);
                mTableLayout.addView(repairRow);
                repairRow.setText("NOTE: "+jObject.optString("note"));
        ...
        else if (result.equals("Failed")||result.equals("0%"))
            resultView.setTextColor(Color.RED);
        tableRow.addView(resultView);
        //type
        String type = jObject.optString("type");
        tableRow.setTag(type);
        if(isTablet)captionView.setTextSize(25);
        if(isTablet)resultView.setTextSize(25);
