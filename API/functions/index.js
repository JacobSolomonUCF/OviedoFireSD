...
function getAllReports(year, callback) {
    ref.once('value').then(rootSnap => {
        ...
        const results = root.forms.results;
        const intervals = root.forms.intervals;
        const templates = root.forms.templates;
            ...
            Object.keys(inventory).forEach(itemType => {
                Object.keys(inventory[itemType]).forEach(item => {
                    ...
                    if(itemType == "vehicles") {
                        ...
                        report.id = item;
                        report.folder = time.weekInterval;
                ...
                ref.child(`users/${req.query.uid}/authentication`).once('value').then(authSnap => {
                    ...
                    if(auth !== null) {
                        if(auth == 0) {
                            ...
                            Promise.all([inventoryPr, resultsPr, intervalsPr, templatesPr]).then(response => {
                                ...
                                const templates = response[3].val();
